...
describe("Patch Roulette API", () => {
    ...
    it("renames users and records ownership by internal identity", async () => {
        ...
        expect((await api("/patches/start", json({ minecraftVersion: "1.21.4", paths: ["renamed.patch"] }))).status).toBe(200);
    ...
    it("initializes each Minecraft version only once", async () => {
        ...
        expect((await api("/patches/init", json({ minecraftVersion: "1.21.4", paths: ["second.patch"] }))).status).toBe(204);
    });
    ...
    it("does not partially claim when a requested path is missing", async () => {
        ...
        const response = await api("/patches/start", json({ minecraftVersion: "1.21.4", paths: ["known.patch", "missing.patch"] }));
    ...
    it("preserves lifecycle duration and ownership rules", async () => {
        ...
        expect((await api("/patches/start", json({ minecraftVersion: "1.21.4", paths: ["lifecycle.patch"] }))).status).toBe(200);
        await new Promise((resolve) => setTimeout(resolve, 2));
        expect((await api("/patches/complete", json({ minecraftVersion: "1.21.4", path: "lifecycle.patch" }))).status).toBe(200);
        expect((await api("/patches/undo", json({ minecraftVersion: "1.21.4", path: "lifecycle.patch" }, "bob"))).status).toBe(200);
        const wrongOwner = await api("/patches/complete", json({ minecraftVersion: "1.21.4", path: "lifecycle.patch" }, "alice"));
        ...
        expect((await api("/patches/cancel", json({ minecraftVersion: "1.21.4", path: "lifecycle.patch" }, "bob"))).status).toBe(200);
    });
    ...
    it("claims an imported legacy account once", async () => {
        ...
        expect({ status: claimed.status, body: await claimed.text() }).toEqual({ status: 204, body: "" });
        expect(await (await api("/patches?minecraftVersion=1.21.4", { headers: access() })).json()).toMatchObject([
    ...
    it("validates legacy imports and rejects a second import", async () => {
        ...
        const futureTimestamp = await api("/import-legacy-data", json(exportPayload([exportPatch({ updatedAt: Date.now() + 60_000 })])));
        ...
        const payload = exportPayload([
            ...
            exportPatch({ path: "second.patch", status: "WIP", duration: 1_000, updatedAt: 1_700_000_003_000 }),
        ]);
        const imported = await api("/import-legacy-data", json(payload));
        expect({ status: imported.status, body: await imported.text() }).toEqual({ status: 204, body: "" });
        expect(await (await api("/patches?minecraftVersion=1.21.4", { headers: access() })).json()).toMatchObject([
    ...
    it("rejects patch records with impossible lifecycle fields", async () => {
        ...
        for (const patch of invalidPatches) expect((await api("/import-legacy-data", json(exportPayload([patch])))).status).toBe(400);
    });
    ...
    it("rejects cross-site unsafe browser requests but permits CLI-style requests without Origin", async () => {
        ...
        const crossSite = await api("/patches/clear", {
            method: "POST",
            headers: { ...access(), Origin: "https://attacker.example", "Content-Type": "application/json" },
        ...
        const cliStyle = await api("/patches/clear", {
            method: "POST",
            headers: { "Cf-Access-Jwt-Assertion": access()["Cf-Access-Jwt-Assertion"], "Content-Type": "application/json" },
            body: JSON.stringify({ minecraftVersion: "1.21.4" }),
        });
