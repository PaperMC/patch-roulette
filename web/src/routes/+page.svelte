<script lang="ts">
    ...
    const versionsQuery = createQuery(() => ({
        ...
        queryFn: () => fetchApi<string[]>("/get-minecraft-versions"),
    }));

    // The backend returns versions newest-last (ORDER BY MAX(p.lastUpdated)),
    ...
    const selectedVersion = $derived(versions.includes(pickedVersion) ? pickedVersion : (versions[versions.length - 1] ?? ""));

    function handleRefresh(): void {
        if (!selectedVersion) return;
        manualRefresh = true;
        Promise.all([
            queryClient.invalidateQueries({ queryKey: queryKeys.patches(selectedVersion), refetchType: "active" }),
            queryClient.invalidateQueries({ queryKey: queryKeys.stats(selectedVersion), refetchType: "active" }),
        ]).finally(() => {
            manualRefresh = false;
        });
    }
...
{#snippet dashboard()}
    <div class="flex flex-wrap items-center gap-3">
        ...
        <VersionSelect {versions} loading={versionsQuery.isPending} value={selectedVersion} onValueChange={(version) => (pickedVersion = version)} />
        <Tabs class="max-w-full min-w-max shrink-0" tabs={views} value={activeView} onValueChange={(view) => (activeView = view)} />
    </div>
    {#if activeView === "stats"}
