<script lang="ts">
    ...
    const progressStatuses = [
        { label: "Done", barClass: "bg-kumo-success", valueClass: "text-kumo-success", getCount: (value: Stats) => value.done },
        { label: "WIP", barClass: "bg-kumo-warning", valueClass: "text-kumo-warning", getCount: (value: Stats) => value.wip },
        { label: "Available", barClass: "bg-kumo-info", valueClass: "text-kumo-info", getCount: (value: Stats) => value.available },
    ];
...
{:else}
    <LayerCard class="min-h-48 max-w-xl">
        ...
        <LayerCard.Primary>
            ...
            {:else if stats && stats.total > 0}
                <div class="bg-kumo-fill mt-4 flex h-2 shrink-0 overflow-hidden rounded-full" role="img" aria-label={barAriaLabel}>
                ...
                <div class="mt-4 flex w-fit items-stretch gap-4" aria-label="Patch progress breakdown" role="list">
                    {#each progressStatuses as status, index (status.label)}
                        ...
                        <div class="flex w-[4.5rem] shrink-0 flex-col gap-1" role="listitem">
                            ...
                            {:else if stats}
                                <Text variant="heading2" as="span" class={`leading-none tabular-nums ${status.valueClass}`}>{status.getCount(stats)}</Text>
