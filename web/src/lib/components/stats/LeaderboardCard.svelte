...
{:else if statsQuery.isPending || stats}
    <LayerCard class="max-w-xl">
        ...
        {#if statsQuery.isPending || (stats && stats.users.length > 0)}
                ...
                <Table layout="fixed" class="text-sm [&_th]:text-sm">
                    ...
                    <Table.Body>
                        {#if statsQuery.isPending}
                            ...
                            {/each}
                        {:else if stats}
                            {#each stats.users as user (user.user)}
                                <Table.Row>
                                    <Table.Cell>
                                        {#if user.rank === 1}
                                        ...
                                        {:else}
                                            <span class="text-kumo-subtle w-6 text-center">#{user.rank}</span>
                                        {/if}
                                    </Table.Cell>
                                    <Table.Cell class="max-w-0 font-medium">
                                        <span class="block max-w-full truncate" title={user.user}>{user.user}</span>
                                    </Table.Cell>
                                    <Table.Cell class="tabular-nums">{user.done}</Table.Cell>
                                    <Table.Cell class="tabular-nums">{user.wip}</Table.Cell>
                                    <Table.Cell class="tabular-nums">{formatDuration(user.timeSpent)}</Table.Cell>
                                </Table.Row>
                            {/each}
                        {/if}
                    </Table.Body>
        ...
        {:else}
            <LayerCard.Primary class="p-0">
                <Empty size="sm" class="border-0 bg-transparent" title="No activity yet" description="User progress will show here once patches are picked up.">
                    {#snippet icon()}
