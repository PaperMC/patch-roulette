<script lang="ts">
    ...
    import type { Column, ColumnDef, Header } from "@tanstack/svelte-table";
    ...
    import type { PatchDetails, PatchStatus } from "$lib/types";
    ...
    /** ISO 8601 duration -> milliseconds; null/invalid -> undefined (sorted last). */
    function durationMs(iso: string | null): number | undefined {
        if (!iso) return undefined;
        const duration = Duration.fromISO(iso);
        return duration.isValid ? duration.as("milliseconds") : undefined;
    }

    /** ISO datetime -> epoch ms; null/invalid -> undefined (sorted last). */
    function updatedAtMs(iso: string | null): number | undefined {
        if (!iso) return undefined;
    ...
    const patchesQuery = createQuery<PatchDetails[]>(() => ({
        queryKey: queryKeys.patches(version),
        queryFn: () => fetchApi<PatchDetails[]>("/get-all-patches", { params: { minecraftVersion: version } }),
        enabled: version !== "",
    ...
    const columns: Array<ColumnDef<typeof features, PatchDetails>> = [
        {
            id: "path",
            accessorKey: "path",
        ...
        },
        {
            ...
            sortFn: (rowA, rowB, columnId) => STATUS_ORDER[rowA.getValue<PatchStatus>(columnId)] - STATUS_ORDER[rowB.getValue<PatchStatus>(columnId)],
        ...
        },
        {
            ...
            sortFn: (rowA, rowB, columnId) => compareNumbers(rowA.getValue<number | undefined>(columnId), rowB.getValue<number | undefined>(columnId)),
            sortUndefined: "last",
            sortDescFirst: true,
        ...
        {
            id: "lastUpdated",
            accessorFn: (row) => updatedAtMs(row.lastUpdated),
            header: (header) => renderSnippet(sortHeader, { label: "Last updated", column: header.column }),
            cell: (info) => renderSnippet(dateCell, { iso: info.row.original.lastUpdated }),
            sortFn: (rowA, rowB, columnId) => compareNumbers(rowA.getValue<number | undefined>(columnId), rowB.getValue<number | undefined>(columnId)),
            sortUndefined: "last",
            sortDescFirst: true,
            enableColumnFilter: false,
...
{#snippet sortHeader(params: { label: string; column: Column<typeof features, PatchDetails, unknown> })}
    <button
        type="button"
    ...
    >
        ...
        {:else}
            <ArrowsDownUp class="text-kumo-muted shrink-0 opacity-0 group-hover/sort-button:opacity-100" size={16} aria-hidden="true" />
        {/if}
    </button>
{/snippet}
...
{#snippet dateCell(params: { iso: string | null })}
    {#if params.iso}
        <span class="block truncate" title={relativeTime(params.iso)}>{formatDateTime(params.iso)}</span>
    {:else}
        —
    {/if}
{/snippet}

{#snippet filterControl(header: Header<typeof features, PatchDetails, unknown>)}
