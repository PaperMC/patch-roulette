<script lang="ts">
    ...
    async function claim(): Promise<void> {
        ...
        try {
            ...
            await Promise.all([queryClient.invalidateQueries({ queryKey: ["patches"] }), queryClient.invalidateQueries({ queryKey: ["stats"] })]);
            open = false;
        } catch (error) {
...
<Dialog bind:open title="Claim legacy account" description="Link patch history from your old Patch Roulette account." class="p-6">
    <form
        novalidate
        class="grid gap-4"
        onsubmit={(event) => {
