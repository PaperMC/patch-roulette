...
export type PatchActionResult = { status: "ok"; patch: Patch } | { status: "missing" } | { status: "conflict"; message: string };
type StartResult = { status: "ok"; patches: Patch[] } | { status: "missing"; path: string } | { status: "conflict"; message: string };
...
export class PatchRoulette extends DurableObject {
    ...
    async init(minecraftVersion: string, paths: string[]): Promise<boolean> {
        ...
        this.db.transaction((tx) => {
            if (tx.select({ path: patches.path }).from(patches).where(eq(patches.minecraftVersion, minecraftVersion)).limit(1).get()) return;

            const timestamp = Date.now();
            for (const path of new Set(paths)) tx.insert(patches).values({ minecraftVersion, path, status: "AVAILABLE", updatedAt: timestamp }).run();
            initialized = true;
        });
        return initialized;
    }
    ...
    async start(minecraftVersion: string, paths: string[], userId: string): Promise<StartResult> {
        const existing = this.db.select({ path: patches.path }).from(patches).where(eq(patches.minecraftVersion, minecraftVersion)).all();
    ...
    async complete(minecraftVersion: string, path: string, userId: string): Promise<PatchActionResult> {
        ...
        if (patch.status !== "WIP") return { status: "conflict", message: `${patchDescription(minecraftVersion, path)} is not WIP.` };
        if (patch.responsibleUserId !== userId)
            return { status: "conflict", message: `You are not responsible for ${patchDescription(minecraftVersion, path)}.` };

        const timestamp = Date.now();
        this.db
            .update(patches)
            .set({ status: "DONE", duration: (patch.duration ?? 0) + timestamp - patch.updatedAt, updatedAt: timestamp })
            .where(and(eq(patches.minecraftVersion, minecraftVersion), eq(patches.path, path)))
            .run();
    ...
    async cancel(minecraftVersion: string, path: string): Promise<PatchActionResult> {
        ...
        if (patch.status !== "WIP" && patch.status !== "DONE")
            return { status: "conflict", message: `${patchDescription(minecraftVersion, path)} is not WIP or DONE.` };
    ...
    async undo(minecraftVersion: string, path: string, userId: string): Promise<PatchActionResult> {
        const patch = this.getPatch(minecraftVersion, path);
        if (!patch) return { status: "missing" };
        if (patch.status !== "DONE") return { status: "conflict", message: `${patchDescription(minecraftVersion, path)} is not DONE.` };
    ...
    async stats(minecraftVersion: string): Promise<Stats> {
        const all = this.db
            ...
            .orderBy(asc(patches.path))
            .all();
        const leaderboard = new Map<string, { userId: string; username: string; wip: number; done: number; intervals: [number, number][] }>();
    ...
    async importLegacyData(records: Patch[], legacyUsers: LegacyUserImport[]): Promise<boolean> {
        ...
        this.db.transaction((tx) => {
            ...
            for (const legacyUser of legacyUsers) {
                ...
                const timestamp = Date.now();
                tx.insert(users).values({ id: userId, username: legacyUser.username, createdAt: timestamp, updatedAt: timestamp }).run();
                tx.insert(legacyCredentials).values({ username: legacyUser.username, userId, passwordHash: legacyUser.passwordHash, disabledAt: null }).run();
    ...
    async claimLegacyUser(currentUserId: string, username: string, password: string) {
        ...
        let claimed = false;
        this.db.transaction((tx) => {
            const activeCredential = tx.select().from(legacyCredentials).where(eq(legacyCredentials.username, username)).get();
            const current = tx.select().from(users).where(eq(users.id, currentUserId)).get();
            if (!current || !activeCredential?.passwordHash || activeCredential.disabledAt !== null || activeCredential.passwordHash !== passwordHash) return;

            tx.update(patches).set({ responsibleUserId: currentUserId }).where(eq(patches.responsibleUserId, activeCredential.userId)).run();
    ...
    async resolveOrProvisionUser(identity: ExternalIdentity) {
        const existingIdentity = this.db
            ...
            .get();
        if (existingIdentity) {
            const existingUser = this.db.select({ id: users.id, username: users.username }).from(users).where(eq(users.id, existingIdentity.userId)).get();
    ...
    async user(userId: string) {
        return this.db.select({ id: users.id, username: users.username }).from(users).where(eq(users.id, userId)).get() ?? null;
    }
