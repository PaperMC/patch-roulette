...
const patchIdSchema = v.strictObject({
  minecraftVersion: minecraftVersionSchema,
  path: nonEmptyString,
});
const patchListSchema = v.strictObject({
  minecraftVersion: minecraftVersionSchema,
  paths: v.pipe(v.array(nonEmptyString), v.minLength(1)),
});
...
const createApi = ({ localIdentity, enforceSameOrigin = true }: ApiOptions = {}) => {
  ...
  api.post(
    "/patches/clear",
    vValidator("json", v.strictObject({ minecraftVersion: minecraftVersionSchema })),
    async (c) => {
      ...
      (result, c) => {
        ...
        for (const user of legacyUsers) {
          if (usernames.has(user.username))
            return c.text(`Duplicate username "${user.username}" in legacy data payload.`, 400);
          usernames.add(user.username);
        }
        const keys = new Set<string>();
        for (const patch of patches) {
          if (patch.updatedAt > Date.now())
            return c.text(`Patch ${patch.minecraftVersion}/${patch.path} has a future updatedAt timestamp.`, 400);
          const key = `${patch.minecraftVersion}\u0000${patch.path}`;
          if (keys.has(key))
            return c.text(`Duplicate patch ${patch.minecraftVersion}/${patch.path} in legacy data payload.`, 400);
          keys.add(key);
        }
      },
    ),
